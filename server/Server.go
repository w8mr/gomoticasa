...
func updateStatus(client mqtt.Client, context *Context) {
	token1 := client.Publish("d566/tempset-ambient/set", 0, false, fmt.Sprintf("%f", context.temperature))
	token2 := client.Publish("d566/tempset-humidity/set", 0, false, fmt.Sprintf("%f", context.humidity))
